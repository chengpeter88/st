...
def run_ml_app():
	...
	with col1:
		...
		genital_thrush = st.select_slider("Genital Thrush", ["No", "Yes"]) 
	...
	with col2:
		...
		### 修改
        #polydipsia = st.select_slider("Polydipsia", ["No", "Yes"])
		partial_paresis = st.selectbox("Partial Paresis", ["No", "Yes"])
		muscle_stiffness = st.select_slider("Muscle Stiffness", ["No", "Yes"]) 
    	# Replacing 'st.radio' with 'st.select_slider' for 'alopecia'
		alopecia = st.select_slider("Alopecia", ["No", "Yes"]) 
		# Replacing 'st.select_slider' with 'st.select_slider' for 'obesity'
		obesity = st.select_slider("Obesity", ["No", "Yes"])
